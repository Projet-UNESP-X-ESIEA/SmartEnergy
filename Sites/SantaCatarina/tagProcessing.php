...
foreach ($datesArray as $value) {
	...
	$dayQuery = "SELECT TIME_TO_SEC(TIME(TIMESTAMP))/86400, ASIN(Radiation/(SELECT MAX(Radiation) FROM weatherstation WHERE DATE(TIMESTAMP) = '$value')) FROM weatherstation WHERE DATE(TIMESTAMP) = '$value' AND TIME(TIMESTAMP) BETWEEN '06:00:00' AND '17:00:00'";
